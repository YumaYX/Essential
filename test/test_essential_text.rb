...
class TestEssentialText < Minitest::Test
  ...
  def test_extract_with_mark
    ...
    pacs = Essential::Text.extract_with_mark(file_content, /header/)
  ...
  def test_extract_with_mark_no_hit
    pacs = Essential::Text.extract_with_mark('CONTENTS', /UNIQUE/)
    assert(pacs.size.eql?(0))
  end
  ...
  def test_extract_with_mark_f
    ...
    pacs = Essential::Text.extract_with_mark_f(temporary_file, /header/)
    assert(!pacs.size.eql?(0))
    pacs.each_with_index do |pac, index|
  ...
  def test_lines_to_hash
    ...
    hash = Essential::Text.lines_to_hash(file_content)
  ...
  def test_lines_to_hash_duplication
    e = assert_raises RuntimeError do
      Essential::Text.lines_to_hash("key a\nkey b")
    end
  ...
  def test_lines_to_hash_f
    ...
    hash = Essential::Text.lines_to_hash_f(temporary_file)
    assert_equal(%w[key0 value0 0], hash['key0'])
    assert_equal(%w[key1 value1 1], hash['key1'])
  end
end
